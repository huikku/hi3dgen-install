@echo off
echo ===== Simple Hi3DGen Installation =====
...
powershell -Command "& {$ErrorActionPreference = 'Stop'; try { Invoke-WebRequest -Uri 'https://github.com/Hi3DGen/Hi3DGen/archive/refs/heads/main.zip' -OutFile '%INSTALL_DIR%\Hi3DGen.zip'; Expand-Archive -Path '%INSTALL_DIR%\Hi3DGen.zip' -DestinationPath '%INSTALL_DIR%' -Force; } catch { Write-Host $_.Exception.Message -ForegroundColor Red; pause; exit 1 } }"
